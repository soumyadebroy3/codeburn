...
`src/providers/index.ts` registers twenty-two providers across two tiers, behind a single data-driven `LAZY_PROVIDERS` table (see `src/providers/index.ts`):

- **Eager**: `claude`, `cline`, `codebuff`, `codex`, `copilot`, `droid`, `gemini`, `ibm-bob`, `kimi`, `kilo-code`, `kiro`, `mistral-vibe`, `openclaw`, `pi`, `omp`, `qwen`, `roo-code`. Imported at module load.
