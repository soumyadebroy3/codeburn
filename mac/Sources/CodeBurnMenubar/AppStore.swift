...
final class AppStore {
    ...
    // dormant on launch when previously bootstrapped, so we don't trigger
    // a keychain prompt unless the user explicitly clicks Connect on the
    // plan tab. Ports upstream PR #338 — was a real UX bug on ad-hoc
    // signed builds because keychain ACLs invalidate on every rebuild and
    // re-prompted on every relaunch. The plan-tab Connect button calls
    // activateClaudeFromDormant() to transition dormant → loading.
    ...
    /// User-initiated. Wakes up a previously-bootstrapped Claude session whose
    /// load state is `dormant` (the on-launch default). Transitions to
    /// `loading` and pulls quota via `refreshSubscriptionReportingSuccess`,
    /// which then touches the keychain. Ports upstream PR #338.
    ...
    /// User-initiated. Wakes up a previously-bootstrapped Codex session whose
    /// load state is `dormant`. Mirrors `activateClaudeFromDormant`.
    ...
    func refreshSubscriptionReportingSuccess() async -> Bool {
        // Dormant means "previously bootstrapped but the user hasn't yet
        // taken an explicit action this session". We refuse to touch the
        // keychain or hit the refresh endpoint until the user clicks
        // Connect, which calls `activateClaudeFromDormant()`.
    ...
    func refreshCodexReportingSuccess() async -> Bool {
        // Same dormant short-circuit as the Claude path — see refreshSubscriptionReportingSuccess.
    ...
    private func claudeQuotaSummary(filter: ProviderFilter) -> QuotaSummary? {
        ...
        if case .dormant = subscriptionLoadState { return nil }
    ...
    private func codexQuotaSummary(filter: ProviderFilter) -> QuotaSummary? {
        ...
        if case .dormant = codexLoadState { return nil }
...
enum SubscriptionLoadState: Sendable, Equatable {
    ...
    case dormant          // previously bootstrapped; keychain not yet accessed this session (no prompt fired)
